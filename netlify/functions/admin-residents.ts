...
export default async (req: Request, context: Context) => {
    ...
    try {
        ...
        if (action === "add") {
            ...
            if (!name || name.length > 120 || room.length > 40) return jsonV2(400, { error: "Le nom ou la chambre est invalide" });
            ...
            if (!resident) return jsonV2(404, { error: "Résident introuvable" });
            ...
            if (idsToDelete.length === 0) return jsonV2(400, { error: "Aucun résident sélectionné" });
            
            for (const id of idsToDelete) {
                await residentsStore.delete(String(id));
            }
            return jsonV2(200, { success: true, residents: residents.filter(r => !idsToDelete.includes(r.id)) });
        } else {
            return jsonV2(400, { error: "Action inconnue" });
        }

        return jsonV2(200, { success: true, residents });
    } catch (error) {
        logFunctionError("admin-residents", error, context.requestId || "unknown");
        return jsonV2(validationStatus(error), { error: error instanceof Error ? error.message : "Erreur lors de la sauvegarde" });
    }
};
